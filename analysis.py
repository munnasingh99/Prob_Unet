    ...
    def create(self, minPx, maxPx, minPlanes, applyWatershed=False, dilation=0):
        ...
        if applyWatershed:
            D = distance_transform_edt(labels > 0)
                        # Ensure the same shape as the original image D
            binary_mask = np.zeros_like(D, dtype=bool)
            
            # Seed generation
            localMax = peak_local_max(D,labels=None,min_distance=0, footprint=np.ones((3,3,3)), exclude_border=1)
            # Convert list of peak coordinates to a binary mask
            binary_mask[tuple(localMax.T)] = True
            print("localMax shape:", localMax.shape)
            print("Structure shape:", np.ones((3,3,3)).shape)

            markers = labelImage(binary_mask, structure=np.ones((3,3,3)))[0]
